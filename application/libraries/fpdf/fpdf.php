...
class FPDF
{
	...
	function SetFont($family, $style = '', $size = 0)
	{
		...
		if (!isset($this->fonts[$fontkey])) {
			...
			if ($family == 'arial')
				$family = 'helvetica';
			if (!empty($this->CoreFonts) && in_array($family, $this->CoreFonts)) {
	...
	function _beginpage($orientation, $size)
	{
		...
		if ($size == '')
			...
			$size = $this->_getpagesize($size);
		if ($orientation != $this->CurOrientation || (!empty($size) && $size[0] != $this->CurPageSize[0]) || (!empty($size) && $size[1] != $this->CurPageSize[1])) {
		...
		if ($orientation != $this->DefOrientation || (!empty($size) && $size[0] != $this->DefPageSize[0]) || (!empty($size) && $size[1] != $this->DefPageSize[1]))
