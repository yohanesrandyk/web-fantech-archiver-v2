...
class Mod_user extends CI_Model
{
    ...
    public function get_user($id =  "%", $division_id = '%')
    {
        return $this->db->query("SELECT user.*, division.name AS division_name FROM user INNER JOIN division ON division.id = user.division_id WHERE user.division_id LIKE ?
        AND user.id LIKE ?", array($division_id, $id))->result_array();
    }
